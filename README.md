...
- prefix(접두어), 태그별칭 사용 예 , <c로 시작


#### 20210610(목)
- if(!save_file_name.isEmpty()) // 게시판 첨부파일 체크시 사용 예정
- 업데이트 실습은 회원암호를 스프링시큐리티5 암호화(1234->해시데이터)로 일괄변경 
- 정방향 암호화, 역방향 복호화는 불가능(스프링시큐리티암호화,MD5 암호화 -데이터베이스 자체 암호화)
- 컨트롤러를 이용해 관리자단 회원관리화면 JSP 만들기.
- GTM시간 (그리니치 천문대 기준시간) - KST한국시간과는 9시간차이
- DB서버에 타임존 설정(Asia/Seoul되어 있으면 그냥 사용)
- 근데 없으면 GTM + 9시간해서 설정해야댐
- 헤로쿠 클라우드 배포시, mysql폴더에 있는 쿼리에 now()를 select date_add(now(3),interval 9 HOUR); 변경 time존이 GTM으로 되어있어서 9시간 추가해줘야됨
- JUNIT 후 회원관리 (CRUD) JSP
...
- JUNIT으로 CRUD 실습
- 마이 바티스를 이용해서 쿼리를 관리
- 마이바티스 추가 순서 : 1. 메이븐(pom.xml)에 추가, 2. 마이바티스 설정바에 추가

#### 20210604(금)
- 오라클일때 : localhost:1521/XE 끝에 있는 접속 URL 경로 XE는 서비스 아이디명,
- 오라클은 사용자명(XE)이 사용자테이블스페이스명, 사용자(XE2)워크스페이스를 추가하면 테이블 스페이스(DB)가 새로 만들어짐. 
- Mysql(마리아 DB) : localhost:3306/XE URL끝의 XE 스키마 명(DB명)
- junit에서는 sql 에러가 보이지만, 콘솔창에는 보이지않음. 그래서 콘솔창에 SQL로그상황이 나오게하는 드라이버를 pon에 추가.

#### 20210603(목)
- 스프링에서 오라클 연동 순서 2가지 : 
ㄴ 1. jdbc(java Database Connection) 확장 모듈을  pom에 추가
ㄴ 2. 오라클 접속 드라이버 확장 모듈을 jar파일을 직접 추가
ㄴ 3. root-context 파일에 오라클 커넥션 빈(스프링 클래스)을 추가
- 스프링 관리하는 클래스 추가 방법 1. : @Controller, @Repository, @Service, 등등...
- 스프링 관리하는 클래스 추가 방법 2. : -context.xml에서 빈(bean 태그)을 추가하면, 스프링 클래스
- JUnit 테스트 : 오라클 연동 후 회원관리 부분 CRUD테스트 
- admin 회원관리(jsp 디자인)부터 프로그램작업 시작 예정.
- 오라클 04장부터 CRUD 실습 예정.
- 1. JUNIT > 마이바티스(DB) >  페이징기능 > 검색기능 > 첨부파일기능 > 스프링시큐리티(로그인인증)
ㄴ 2. > 댓글기능(RestAPI-백엔드,Ajax처리 - 프런트단)>네이버아이디로그인 (외부 API사용) > 헤로쿠 클라우드 배포 >
- 3. 문서작업(화면기획서 XLS제작, 화면 설계서 PPT 제작)

#### 20210602(수)
- 예외처리의 목적 : 에러상황에서도 프로그램이 중단되지 않게 하기위해서
- 스프링에서는 개발자가 처리하는 경우는 많이 없음.
- 스레드 : thread, 실행되는 프로그램을 명시. 1개의 프로그램에는 보통 1개의 스레드가 실행되지만 1개의 프로그램에 여러 스레드가 실행될 경우도 있다 . ex) 네트워크 프로그램의 경우 - 다운로드 버튼 실행후(용량 많을 때) 리스트버튼 클릭했을 때 목록 보기 스레드가 실행됨, 멀티스레드라고함. 안드로이드 앱에서 필요
- 예외처리 : ERROR 발생시 프로그램이 중지 되는 것을 방지, 계속 프로그램을 사용할 수 있도록 처리.
ㄴ EXCEPTION, Throwable(오브젝트가 실행될 시 에러가 발생하면, 예외내용를 보낼 때 사용하는 클래스)
- 스프링앱에서는 예외정보를 스프링 처리하지만, 자바앱에서는 예외 정보를 개발자가 처리해야함
- packge는 관련기능을 한 곳에 모아서 관리하기 편하게 하기위해 구분한 이름.
...
- Controller 클래스에서 생성한 변수 사용(자바) model 객체로 전송 -> jsp 출력 -> 전송은 스프링이 처리함
- 출력하는 방식 : EL(Express Language방식: ${변수명}, <%=${변수} %> 
- 많이 사용하는 방식 : JSTL방식(*표준). 출력, 반복, 조건 등등 다양한 방식
- 언어 배우는 순서 : 주석 > 디버그 > 변수 > 메서드 > 클래스 > MVC 모델
- 디버그 방법 : 자바(System.out 이용해서 콘솔창에 출력)
- 스프링에서는 logger 사용
- 4장 패키지와 예외처리 실습.
- "/home -> "/resources/home
- '/home -> '/resources/home
- 스프링이 관리하는 클래스(bean)의 종류 3(개발자가 만드는 클래스 상단에 입력하면 사용가능)
ㄴ 1.@Controller 클래스빈 : 라우터 역할 메서드가 위치
ㄴ 2.@Service 클래스빈       : 비즈니스 로직에 들어감
ㄴ 3.@Repository        : model처리 메서드를 만듬
- 위 3가지의 @를 사용하지 않는 클래스는 DI(객체생성-의존성주입)x, AOP, IOC 사용불가
