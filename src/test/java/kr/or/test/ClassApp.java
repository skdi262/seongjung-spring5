package kr.or.test;
/**
 * 이 클래스는 클래스 상속(extends)에 연습,
 * 오브젝트(객체)생성과 new 키워드(예약어) 생성자메서드 사용 연습,
...
class Circle {
	...
	public Circle(int radius) {
		r = radius;//Get|Set중에 섯 저장처리
	}
	//원의 넓이를 구하는 메서드 반환(리터)값의 크기가 더블형(아래)
...
class Salesman extends Employee {//exetends확장 -> 상속
	public Salesman() {//클래스명과 똑같은 이름의 메서드가 생성자 함수입니다.
		// 자동으로 생성되는데, 개발자가 커스터마이징 할때 만듭니다. (아래)
		mDept = "판매 부서";//부모클래스의 변수를 상속받아 서 사용가능.
	}
	public void doJob() {
...
abstract class GraphicObject {
	...
	//추상메서드를 만드는 이유는 메서드가 수십개 수백개일때 개발자 구현할떄 어려움이 존재
...
class Traiangle extends GraphicObject {
	...
	@Override //부모클래스의 메서드를 재정의해서 사용 = 오버라이드 / 오버로드 차이점확인()
	void draw() {
		// 삼각형을 만듬
		System.out.println("  *");
		System.out.println(" * * ");
...
public class ClassApp {
	...
	public static void main(String[] args) {
		...
		GraphicObject rectangleObiect = new Rectangle();
		traiangleObject.draw();
		rectangleObiect.draw();
